...
public class Lloguer {
	...
	public double unitatDeCostPerTarifa() {
		double unitatDeCostTotal = 0;
		switch (this.getVehicle().getCategoria()) {
			case Vehicle.BASIC:
				unitatDeCostTotal += UNITAT_DE_COST_BASIC;
				if (this.getDies() > NOMBRE_DE_DIES_INICIALS_BASIC) {
					unitatDeCostTotal += (this.getDies() - NOMBRE_DE_DIES_INICIALS_BASIC) * TARIFA_DE_COST_BASIC;
				}
				break;
		    case Vehicle.GENERAL:
				unitatDeCostTotal += UNITAT_DE_COST_GENERAL;
				if (this.getDies() > NOMBRE_DE_DIES_INICIALS_GENERAL) {
					unitatDeCostTotal += (this.getDies() - NOMBRE_DE_DIES_INICIALS_GENERAL) * TARIFA_DE_COST_GENERAL;
		        }
				break;
			case Vehicle.LUXE:
				unitatDeCostTotal += this.getDies() * UNITAT_DE_COST_LUXE;
				break;
		}
		return unitatDeCostTotal;
	}
